...
public abstract class PayPalResource ////: PayPalSerializableObject
{
    ...
    internal static T ConfigureAndExecute<T>(IPayPalClient apiContext, string httpMethod, string resource, T payload,
        string? endpoint = null, bool setAuthorizationHeader = true, bool maskRequestId = false)
    {
        return ConfigureAndExecute<T>(apiContext, httpMethod, resource, (object?)payload, endpoint, setAuthorizationHeader, maskRequestId);
    }
    ...
    /// <exception cref="T:SharedKernel.Infrastructure.PayPal.PaymentsException">Thrown if an HttpException was raised and contains a Payments API error object.</exception>
    /// <exception cref="T:SharedKernel.Infrastructure.PayPal.PayPalException">Thrown for any other issues encountered. See inner exception for further details.</exception>
    internal static T ConfigureAndExecute<T>(IPayPalClient apiContext, string httpMethod, string resource,
        PatchRequest payload, string? endpoint = null, bool setAuthorizationHeader = true, bool maskRequestId = false)
    {
        return ConfigureAndExecute<T>(apiContext, httpMethod, resource, (object?)payload, endpoint, setAuthorizationHeader, maskRequestId);
    }
    ...
    /// <exception cref="T:SharedKernel.Infrastructure.PayPal.PayPalException">Thrown for any other issues encountered. See inner exception for further details.</exception>
    internal static T ConfigureAndExecute<T>(IPayPalClient apiContext, string httpMethod, string resource,
        object? payload = null, string? endpoint = null, bool setAuthorizationHeader = true,
        bool maskRequestId = false)
    {
        return apiContext.Send<T>(httpMethod, resource, payload, endpoint, setAuthorizationHeader);
    }
}