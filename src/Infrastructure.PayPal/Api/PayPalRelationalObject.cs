...
public class PayPalRelationalObject : PayPalResource
{
    ...
    public string GetApprovalUrl(bool setUserActionParameter = false)
    {
        ...
        return hateoasLink != null ? hateoasLink.Href + (setUserActionParameter ? "&useraction=commit" : "") : string.Empty;
    }
    ...
    public string GetTokenFromApprovalUrl()
    {
        ...
        return !string.IsNullOrEmpty(approvalUrl) ? HttpUtility.ParseQueryString(new Uri(approvalUrl).Query).Get("token") : string.Empty;
    }
}